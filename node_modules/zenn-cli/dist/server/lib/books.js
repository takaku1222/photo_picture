...
function getLocalBookMeta(slug) {
    ...
    var errorFilename = "books/".concat(slug, "/config.yaml");
    if (specifiedChapterSlugs) {
        if (!Array.isArray(specifiedChapterSlugs)) {
            Log.error("".concat(errorFilename, "\u306E chapters \u306B\u306Fslug\u306E\u914D\u5217\u306E\u307F\u3092\u6307\u5B9A\u3067\u304D\u307E\u3059"));
            return null;
        }
        var anyNonStringSlug = specifiedChapterSlugs.some(function (slug) { return typeof slug !== 'string'; });
        if (anyNonStringSlug) {
            Log.error("".concat(errorFilename, "\u306E chapters \u306B\u306F\u6587\u5B57\u5217\u306Eslug\u306E\u307F\u3092\u542B\u3081\u308B\u3053\u3068\u304C\u3067\u304D\u307E\u3059\u3002"));
            return null;
        }
        ...
        if (anyInvalidFormatSlug) {
            Log.error("".concat(errorFilename, "\u306E chapters \u306B\u306F\u4E00\u6B21\u5143\u914D\u5217\u306E\u307F\u3092\u6307\u5B9A\u3067\u304D\u307E\u3059\u3002\u30CD\u30B9\u30C8\u306F\u3067\u304D\u307E\u305B\u3093"));
            return null;
        }
    }
    return bookMeta;
...
function getLocalChapter(book, chapterFilename) {
    ...
    var rawHtml = (0, zenn_markdown_html_1.default)(bodyMarkdown);
    var bodyHtml = (0, helper_1.completeHtml)(rawHtml);
...
function getBookSlugs(sort) {
    var dirFullpath = (0, helper_1.getWorkingPath)('books');
...
function readBookFile(slug) {
    ...
    try {
        ...
        console.log(e);
        Log.error("config.yaml\u306E\u8868\u8A18\u306B\u8AA4\u308A\u304C\u3042\u308A\u307E\u3059\uFF08books/".concat(slug, "\uFF09"));
        return null;
    }
}
function getBookFullDirpath(slug) {
    return (0, helper_1.getWorkingPath)("books/".concat(slug));
}
function getBookConfigRaw(slug) {
    ...
    var fileRaw = (0, helper_1.getFileRaw)(fullpath) || (0, helper_1.getFileRaw)(fallbackFullpath);
    if (!fileRaw) {
        Log.error("".concat(fullpath, "\u306E\u5185\u5BB9\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F"));
    }
    return fileRaw;
}
function readCoverFile(imageFullpath) {
    try {
        var bufferImage = (0, helper_1.getImageRaw)(imageFullpath);
        if (!bufferImage)
            return null;
        var coverFilesize = (0, helper_1.getImageSize)(imageFullpath);
        var _a = (0, image_size_1.default)(bufferImage), width = _a.width, height = _a.height;
        var mediaType = imageFullpath.endsWith('.png')
            ? 'image/png'
            : 'image/jpeg';
        var coverDataUrl = (0, helper_1.bufferToDataURL)(bufferImage, mediaType);
        return {
...
function getChapterFilenames(bookSlug) {
    var dirpath = (0, helper_1.getWorkingPath)("books/".concat(bookSlug));
    var allFiles = (0, helper_1.listFilenames)(dirpath);
    if (allFiles === null) {
        Log.error("".concat(dirpath, "\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F"));
        return [];
    }
...
function readChapterFile(book, filename) {
    ...
    if (!raw)
        return null;
    var _b = (0, gray_matter_1.default)(raw), data = _b.data, bodyMarkdown = _b.content;
    return {
...
function getChapterFileRaw(bookSlug, chapterFilename) {
    var fullpath = (0, helper_1.getWorkingPath)("books/".concat(bookSlug, "/").concat(chapterFilename));
    var fileRaw = (0, helper_1.getFileRaw)(fullpath);
    if (!fileRaw) {
        Log.error("".concat(fullpath, "\u306E\u5185\u5BB9\u3092\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F"));
    }
    return fileRaw;
